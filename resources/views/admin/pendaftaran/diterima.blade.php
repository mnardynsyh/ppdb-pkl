...
<div x-data="{ 
    ...
    <div class="max-w-7xl mx-auto w-full flex-1 flex flex-col gap-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-neutral-900">Data Pendaftar Diterima</h1>
                <p class="text-sm text-neutral-500 mt-1">Daftar calon siswa yang telah lolos seleksi dan verifikasi.</p>
            </div>
            <div class="hidden sm:block">
                <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary-50 text-primary-700 text-xs font-bold border border-primary-200">
        ...
        <div class="flex flex-col gap-4">
            ...
            <form action="{{ route('admin.pendaftaran.diterima') }}" method="GET" 
                  class="bg-white p-2 rounded-xl border border-neutral-200 shadow-sm flex items-center gap-2 transition-all focus-within:border-primary-500 focus-within:ring-4 focus-within:ring-primary-500/10">
                
                <button type="submit" class="w-10 h-10 flex items-center justify-center text-neutral-400 hover:text-primary-600 transition-colors rounded-lg hover:bg-neutral-50">
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-neutral-100">
                        @forelse($siswas as $i => $siswa)
                            <tr class="hover:bg-primary-100/30 transition-colors group">
                                {{-- No --}}
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-4">
                                        <div class="w-10 h-10 rounded-xl bg-primary-50 text-primary-700 flex items-center justify-center text-sm font-bold shrink-0 border border-primary-200">
                                            {{ substr($siswa->nama_lengkap, 0, 2) }}
                                        </div>
                                        <div>
                                            <p class="text-sm font-bold text-neutral-900 group-hover:text-primary-700 transition-colors">{{ $siswa->nama_lengkap }}</p>
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-neutral-500 text-sm font-medium">Belum ada siswa yang diterima.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            <div class="lg:hidden space-y-4 mb-6">
                    ...
                    <div class="bg-white rounded-2xl p-5 border border-neutral-200 shadow-sm relative overflow-hidden">
                        ...
                        <div class="pl-3">
                            <div class="flex justify-between items-start mb-3">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-xl bg-primary-50 text-primary-700 flex items-center justify-center text-sm font-bold border border-primary-200">
                                        {{ substr($siswa->nama_lengkap, 0, 2) }}
                                    </div>
                                    <div>
                                ...
                                <span class="text-[10px] font-bold px-2 py-1 bg-primary-50 text-primary-700 rounded border border-primary-200">
                                    Diterima
                                </span>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-2 pt-2">
                                ...
                                {{-- Batalkan --}}
                                <button @click="openModal('batalkan', {{ $siswa->id }}, '{{ $siswa->nama_lengkap }}')" class="w-full py-2 bg-yellow-50 border border-yellow-200 text-yellow-700 rounded-lg text-xs font-bold hover:bg-yellow-100 transition-colors flex items-center justify-center gap-2">
                ...
                @empty
                    <div class="bg-white rounded-2xl border border-neutral-200 shadow-sm p-8 text-center">
                        <p class="text-sm font-medium text-neutral-500">Belum ada siswa yang diterima.</p>
                    </div>
                @endforelse
            </div>
    ...
    <div x-show="activeModal === 'batalkan'" style="display: none;" 
        ...
        <div class="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl transform transition-all"
            ...
            <div class="text-center">
                ...
                <h3 class="text-lg font-bold text-neutral-900">Batalkan Penerimaan?</h3>
            ...
            <div class="mt-6 flex gap-3">
                ...
                <button @click="closeModal()" 
                    class="w-full rounded-xl border border-neutral-200 bg-white py-2.5 text-sm font-bold text-neutral-600 
                ...
                <form :action="'{{ url('admin/pendaftaran') }}/' + selectedId + '/batalkan'" 
                    ...
                    <button type="submit" 
                        class="w-full rounded-xl bg-yellow-600 py-2.5 text-sm font-bold text-white 
                               hover:bg-yellow-700 transition-colors shadow-lg shadow-yellow-200">
                        Ya, Batalkan
                    </button>
                </form>
            </div>
