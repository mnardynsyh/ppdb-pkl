            ...
            <tbody>
                @forelse($agama as $i => $a)
                    <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <td class="px-6 py-2 font-semibold">{{ $a->agama }}</td>
                        <td class="px-6 py-2">
                            <div class="flex items-center gap-3">
                                <button type="button"
                                    onclick="openEditModal({{ $a->id }}, @js($a->agama))"
...
<div id="addModal"
    ...
    <div class="absolute left-1/2 top-1/2 w-[22rem] -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white p-6 shadow-xl dark:bg-gray-800">
        <div class="mb-4 flex items-start justify-between">
            <h2 class="text-lg font-semibold text-gray-800 dark:text-gray-100">Tambah Agama</h2>
        ...
        <form action="{{ route('admin.agama.store') }}" method="POST">
            @csrf
            <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Agama</label>
...
<div id="editModal"
    ...
    <div class="absolute left-1/2 top-1/2 w-[22rem] -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white p-6 shadow-xl dark:bg-gray-800">
        <div class="mb-4 flex items-start justify-between">
            <h2 class="text-lg font-semibold text-gray-800 dark:text-gray-100">Edit Agama</h2>
        ...
        <form id="editForm" method="POST">
            ...
            <input type="text" name="agama" id="editAgama" required
                   class="mb-4 w-full rounded border border-gray-300 p-2 text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
            <div class="flex justify-end gap-2">
...
<script>
    ...
    const editInput = document.getElementById('editAgama');

    function openAddModal()  { addModal.classList.remove('hidden'); }
    function closeAddModal() { addModal.classList.add('hidden'); }

    function openEditModal(id, agama) {
        editInput.value = agama;
        editForm.action = `/admin/agama/${id}`;
