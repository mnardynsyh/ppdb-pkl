...
<div x-data="{ editing: {{ $errors->any() ? 'true' : 'false' }} }"
    ...
    <div class="max-w-7xl mx-auto w-full flex-1 flex flex-col lg:justify-center">
        ...
        {{-- NOTE: alert lama dihilangkan — SweetAlert akan menampilkan notifikasi --}}
        {{-- 2. CONTENT AREA --}}
                                        ...
                                        <tr class="hover:bg-blue-50/30 transition-colors group">
                                            ...
                                            <td class="px-6 py-4 text-right">
                                                <div class="flex items-center justify-end gap-2">
                                                    ...
                                                    <form action="{{ route('admin.jadwal.destroy', $jadwal) }}" method="POST" onsubmit="event.preventDefault(); confirmDelete(this)">
                                                        @csrf
                                                        @method('DELETE')
                    ...
                    <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="title" required placeholder="Masukkan judul kegiatan" class="w-full rounded-lg border-slate-200 text-sm focus:border-blue-500 focus:ring-0 font-medium placeholder-slate-300">
                            </div>
                            <div class="grid grid-cols-3 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="order" value="1" required placeholder="Urutan tampil" class="w-full rounded-lg border-slate-200 text-sm focus:border-blue-500 focus:ring-0 font-medium text-center">
                                </div>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Deskripsi Singkat</label>
                                <textarea name="description" rows="2" placeholder="Tambahkan deskripsi singkat (opsional)" class="w-full rounded-lg border-slate-200 text-sm focus:border-blue-500 focus:ring-0 font-medium placeholder-slate-300"></textarea>
                            </div>
                        </div>
                    </div>
                ...
                <form id="editForm" method="POST">
                    ...
                    <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        ...
                        <div class="space-y-4">
                            <div>
                                <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Judul Kegiatan</label>
                                <input type="text" name="title" id="edit_title" required placeholder="Masukkan judul kegiatan" class="w-full rounded-lg border-slate-200 text-sm focus:border-blue-500 focus:ring-0 font-medium">
                            </div>
                            <div class="grid grid-cols-3 gap-4">
                                <div class="col-span-2">
                                    <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Tanggal / Rentang</label>
                                    <input type="text" name="date_range" id="edit_date_range" required placeholder="Contoh: 10 - 15 Juli" class="w-full rounded-lg border-slate-200 text-sm focus:border-blue-500 focus:ring-0 font-medium">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Urutan</label>
                                    <input type="number" name="order" id="edit_order" required placeholder="Urutan tampil" class="w-full rounded-lg border-slate-200 text-sm focus:border-blue-500 focus:ring-0 font-medium text-center">
                                </div>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-500 uppercase mb-1">Deskripsi Singkat</label>
                                <textarea name="description" id="edit_description" rows="2" placeholder="Tambahkan deskripsi singkat (opsional)" class="w-full rounded-lg border-slate-200 text-sm focus:border-blue-500 focus:ring-0 font-medium"></textarea>
                            </div>
                        </div>
                    </div>
...
{{-- SCRIPTS --}}
@push('styles')
<!-- SweetAlert animation style (small slide-up + scale + fade) -->
<style>
.swal-animate-in {
    animation: swalIn 0.32s cubic-bezier(0.22, 1, 0.36, 1);
}
...
<script>
    ...
    // SweetAlert: show success / validation error from server
    document.addEventListener("DOMContentLoaded", function() {
        @if(session('success'))
            Swal.fire({
                ...
                customClass: { popup: 'swal-animate-in' }
            });
        @endif
...
<!-- SweetAlert2 CDN (ensure available) -->
