...
<div class="flex flex-col w-full min-h-screen font-sans text-neutral-800 overflow-x-hidden">

    <section class="relative w-full min-h-[100svh] lg:h-screen bg-neutral-900 overflow-hidden group" 
             x-data="carousel()"
        ...
        <div class="absolute inset-0 w-full h-full">
            <template x-for="(slide, index) in slides" :key="index">
                <div class="absolute inset-0 w-full h-full transition-transform duration-700 ease-[cubic-bezier(0.25,1,0.5,1)]"
                    ...
                    <img :src="slide.image" class="w-full h-full object-cover brightness-[0.55]" />
                        ...
                        <div class="max-w-5xl w-full space-y-6 lg:space-y-8 mt-10 sm:mt-0">
                            ...
                            <div x-show="activeSlide === index"
                                ...
                                <span class="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-white/10 border border-white/20 backdrop-blur-md text-white text-[10px] sm:text-xs font-bold tracking-widest uppercase">
                                    <span class="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-primary-400 animate-pulse"></span>
                            ...
                            {{-- Deskripsi --}}
                            <p class="text-sm sm:text-base md:text-lg lg:text-xl text-neutral-200 max-w-2xl mx-auto font-light leading-relaxed px-2"
                               x-show="activeSlide === index"
                            ...
                            <div class="flex flex-col sm:flex-row justify-center gap-3 sm:gap-4 pt-4 sm:pt-8 w-full px-4 sm:px-0"
                                ...
                                <template x-for="btn in slide.buttons">
                                    <a :href="btn.href"
                                       ...
                                       :class="btn.isPrimary 
                                            ? 'bg-primary-600 text-white hover:bg-primary-700 shadow-lg shadow-primary-900/30' 
        ...
        <button @click="prevSlide()" 
                class="absolute top-1/2 left-4 sm:left-8 z-20 w-12 h-12 flex items-center justify-center rounded-full border border-white/30 text-white hover:bg-white hover:text-neutral-900 transition-all -translate-y-1/2 focus:outline-none hidden lg:flex backdrop-blur-sm group-hover:border-white/50">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
        </button>
        <button @click="nextSlide()" 
                class="absolute top-1/2 right-4 sm:right-8 z-20 w-12 h-12 flex items-center justify-center rounded-full border border-white/30 text-white hover:bg-white hover:text-neutral-900 transition-all -translate-y-1/2 focus:outline-none hidden lg:flex backdrop-blur-sm group-hover:border-white/50">
        ...
        <div class="absolute bottom-6 sm:bottom-10 left-0 right-0 flex justify-center gap-2 sm:gap-3 z-20">
            <template x-for="(slide, index) in slides">
                <button @click="goToSlide(index)" 
                        ...
                        :class="activeSlide === index ? 'w-8 sm:w-10 bg-primary-500' : 'w-2 sm:w-2 bg-white/40 hover:bg-white/80'">
                </button>
            </template>
        </div>
    </section>
        ...
        slides: [
            {
                ...
                badge: "PPDB {{ $tahun_ajaran ?? date('Y') . '/' . (date('Y')+1) }}",
                title: 'Penerimaan Peserta <br> <span class="text-primary-400">Didik Baru</span>',
            ...
            },
            {
                ...
                title: 'Kembangkan Bakat <br> <span class="text-primary-300">Non-Akademik</span>',
            ...
            },
            {
                ...
                title: 'Sarana Belajar <br> <span class="text-primary-300">Modern & Lengkap</span>',
