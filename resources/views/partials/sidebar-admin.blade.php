            ...
            <div class="flex items-center ms-3">
                <div>
                    <button type="button" class="flex text-sm bg-gray-800 rounded-full focus:ring-4 focus:ring-blue-100 transition-all" aria-expanded="false" data-dropdown-toggle="dropdown-user">
                        ...
                        <img class="w-9 h-9 rounded-full object-cover border-2 border-white shadow-sm" 
                            src="{{ Auth::user()->foto ? Storage::url(Auth::user()->foto) : 'https://ui-avatars.com/api/?name='.urlencode(Auth::user()->name).'&background=3B82F6&color=FFFFFF&bold=true' }}" 
                            alt="user photo">
                    </button>
                </div>
                {{-- Dropdown Content --}}
                <div class="z-50 hidden my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-lg shadow-xl border border-gray-100 w-56" id="dropdown-user">
                    <div class="px-4 py-3 bg-gray-50 rounded-t-lg" role="none">
                        ...
                        <p class="text-xs text-gray-500 truncate font-medium" role="none">
                            {{ Auth::user()->email }}
                        </p>
                    </div>
        ...
        <ul class="space-y-1 font-medium">
            ...
            <li x-data="{ open: {{ request()->routeIs('admin.pendaftaran.*') ? 'true' : 'false' }} }">
                ...
                <ul x-show="open" 
                    ...
                    {{-- LINE CONNECTOR (Garis Vertikal) --}}
                    {{-- Posisi left-[29px] disesuaikan agar lurus dengan tengah ikon parent --}}
            ...
            {{-- Dropdown Pengaturan --}}
            <li x-data="{ open: {{ request()->routeIs(['admin.pengaturan.*', 'admin.jadwal.*']) ? 'true' : 'false' }} }">
                <button @click="open = !open" 
                        class="flex items-center w-full px-4 py-3 transition-all duration-200 border-l-[6px] text-[15px] group relative z-10
                    ...
                    <i class="fa-solid fa-gears w-5 text-center text-[18px] transition duration-200 
                       {{ request()->routeIs(['admin.pengaturan.*', 'admin.jadwal.*']) ? 'text-blue-400' : 'text-slate-500 group-hover:text-slate-300' }}"></i>
                    
                    <span class="flex-1 ms-3 text-left whitespace-nowrap tracking-wide">Sistem</span>
                ...
                <ul x-show="open" 
                    ...
                    <li>
                        <a href="{{ route('admin.pengaturan.index') }}" 
                           ...
                           <i class="fa-solid fa-toggle-on w-4 text-center text-[12px] mr-3 transition-colors
                              {{ request()->routeIs('admin.pengaturan.*') ? 'text-blue-400' : 'text-slate-600 group-hover:text-slate-400' }}"></i>
                    ...
                    <li>
                        <a href="{{ route('admin.jadwal.index') }}" 
                           ...
                           <i class="fa-solid fa-calendar-days w-4 text-center text-[12px] mr-3 transition-colors
                              {{ request()->routeIs('admin.jadwal.*') ? 'text-blue-400' : 'text-slate-600 group-hover:text-slate-400' }}"></i>
